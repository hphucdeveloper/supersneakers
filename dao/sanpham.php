...
function sanpham_update($id_category, $name, $price, $img, $id){
	if($img!=""){
		$sql = "UPDATE product SET id_category=?, name=?, price=?, img=? WHERE id=?";
		pdo_execute($sql, $id_category, $name, $price, $img, $id);
	} else {
		$sql = "UPDATE product SET id_category=?, name=?, price=? WHERE id=?";
		pdo_execute($sql, $id_category, $name, $price, $id);
	}
}
