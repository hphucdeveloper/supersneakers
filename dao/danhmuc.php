...
function show_dm($dsdm, $id_category){
    $html_dmlist='';
	foreach ($dsdm as $dm) {
		extract($dm);
        if($id===$id_category){
		...
		$html_dmlist.='<option value="'.$id.'" '.$se.'>'.$name.'</option>';
	}
    return $html_dmlist;
}
    ...
    $i = 1;
                                foreach ($dsdm as $dm) {
                                    ...
                                    $html_dmadmin.='<tr>
                                            ...
                                            <td>'.$name.'</td>
                                            <td>
                                                <a href="index.php?pg=delcat&id='.$id.'" onclick="return confirm(\'Bạn chắc chắn muốn xóa danh mục này?\')" class="btn btn-danger"><i
